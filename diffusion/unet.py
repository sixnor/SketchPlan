# From diffusion policy: https://github.com/real-stanford/diffusion_policy
# ### **Network**
# 
...
class CondEncoder(nn.Module):
    def __init__(self, cond_dim, cond_channels, dropout=0.0):
        super().__init__()
...
class ConditionalResidualBlock1D(nn.Module):
    def __init__(self,
        ...
        self.cond_encoder = CondEncoder(cond_dim, cond_channels, dropout=dropout)
...
class ConditionalUnet1D(nn.Module):
    ...
    def freezeNonCond(self):
        ...
        for module in self.modules():
            if isinstance(module, CondEncoder):
                for param in module.parameters():   
    ...
    def forward(self,
        ...
        if global_cond is not None: # If we don't condition on inputs
...
class SequenceChunkDropout(nn.Module):
    def __init__(self, drop_len: int, p: float = 0.5):
        super().__init__()
    ...
    def forward(self, x):
        """
        x: (T, d) or (B, T, d) tensor
        """
        if not self.training or self.p == 0.0:
            return x

        if x.dim() == 2:  # (T, d)
            x = self._drop_chunk(x)
        elif x.dim() == 3:  # (B, T, d)
            x = torch.stack([self._drop_chunk(seq) for seq in x], dim=0)
        else:
            raise ValueError("Input must be (T, d) or (B, T, d)")
        return x

    def _drop_chunk(self, x):
        T, d = x.shape
        if T <= self.drop_len or torch.rand(1).item() > self.p:
            return x
        start = torch.randint(0, T - self.drop_len + 1, (1,)).item()
        mask = torch.ones(T, device=x.device)
        mask[start:start + self.drop_len] = 0
        return x * mask[:, None]