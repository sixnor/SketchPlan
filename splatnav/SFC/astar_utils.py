...
def astar3D(field, source, target, feasible):
    ...
    #Performs A* 
    path = dijkstra3d.binary_dijkstra(field, source, target, connectivity=6, background_color=1)
    path = path.astype(np.int32)

    print(f'Returning path of length {len(path)}')
 
    traj = feasible[path[:, 0], path[:, 1], path[:, 2]]
